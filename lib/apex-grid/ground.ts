...
export async function gatherGround(input: ApexInput): Promise<GroundContext> {
  ...
  try {
    ...
    return {
      ...
      findings: [],
      digest:
        "Insight Base: błąd połączenia z bazą nastawień grup społecznych — dalsze warstwy mają jawnie zaznaczyć brak danych o gruncie społecznym, nie zgadywać nastrojów.",
    };
  }
...
function emptyDigest(query: string): string {
  return `Insight Base${query ? ` (zapytanie: „${query.slice(0, 120)}")` : ""}: brak dopasowanych badań i syntez o nastawieniu grup społecznych do tej sprawy. Wnioski o reakcjach grup formułuj wyłącznie jako hipotezy do sprawdzenia, jawnie oznaczone — nie jako dane.`;
}

function buildGroundDigest(syntheses: GroundSynthesis[], findings: GroundFinding[]): string {
  const lines: string[] = ["Insight Base (realne badania i sondaże o grupach społecznych):"];
